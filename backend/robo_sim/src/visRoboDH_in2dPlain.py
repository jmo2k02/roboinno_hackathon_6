...
def build_robot(model: str):
    ROBO_DICT: dict[str, Any] = {
        "panda": rtb.models.Panda,
        "ur10": rtb.models.UR10,
        "frankie": rtb.models.Frankie
    }
    robo = ROBO_DICT.get(model, rtb.models.Panda)()

    if model == "panda":
        robo.q = robo.qr
        n_dof = 7 
    elif model == "ur10":
        robo.q = np.array([0, -np.pi/2, np.pi/2, -np.pi/2, -np.pi/2, np.pi/2])
        n_dof = 6
    
    return robo, n_dof
    

def runPreview_in2dPlain(model_key: str):
    ...
    # add robot 
    # panda = ROBO_DICT.get(model_key.upper(), rtb.models.Panda)()
    # panda.q = panda.qr

    panda, n_dof = build_robot(model_key)
    backend.add(panda)
    ...
    if draw == "svg":
        ...
        # Load file
        file_path = Path(__file__).parent.parent / "src" / "data" / "robot_path_.csv"  
    ...
    tool_orientation = sg.Arrow(0.04,0.0055,0.24,0.30, color = [0,0.7569,0.7137])
